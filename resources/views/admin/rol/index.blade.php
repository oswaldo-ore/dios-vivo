...
@section('menu-top')
    <!--begin::Item-->
    <li class="breadcrumb-item text-dark">Gestionar rol</li>
    <!--end::Item-->
@endsection
            ...
            <div class="col-md-10 col-sm-12">
                <div class="table-responsive">
                    <table
                        ...
                        <tbody>
                            @forelse ($rols as $rol)
                                <tr id="rol_{{ $rol->id }}" class="text-dark fw-bold fs-5">
                                    <td> {{ $rol->id }}</td>
                                    <td> {{ $rol->name }}</td>
                                    <td>
                                        <a href="#"
                                            class="btn btn-sm {{ $rol->is_enabled ? 'btn-danger ' : 'btn-primary ' }}  btn-icon"
                                            data-bs-toggle="modal" data-bs-target="#rol_change_state{{ $rol->id }}">
                                            <i
                                                class="fas {{ $rol->is_enabled ? 'fa-thumbs-down' : 'fa-thumbs-up ' }}  "></i>
                                        </a>
                                        <a href="#" class="btn btn-sm btn-light-primary btn-icon pe-1"
                                            data-bs-toggle="modal" data-bs-target="#rol_edit{{ $rol->id }}">
                                        ...
                                        <a class="btn btn-sm btn-light-danger btn-icon" data-bs-toggle="modal"
                                            data-bs-target="#rol_delete{{ $rol->id }}">
                                            <span class="svg-icon svg-icon-primary svg-icon">
