...
<script>
    var year = "2022";
    let startDate = @json($dateInicio);
    let endDate = @json($dateFin);
    $("#close_box").click(function() {
        Swal.fire({
            html: `Estas seguro que desea cerrar caja entre <strong>${startDate}</strong> a <strong>${endDate}</strong> <br> <strong class="text-uppercase">No se puede revertir</strong>`,
            icon: "warning",
    ...
    function closeBoxMonthly() {
        var url = "{{ route('monthly.closure.box.close') }}";
        $.ajaxSetup({
            headers: {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.codigo == 1) {
                    Swal.fire({
                        html: `CAJA CERRADA <strong>Rango de fecha ${startDate} a ${endDate}</strong>`,
                        icon: "success",
                        buttonsStyling: false,
                        allowOutsideClick: false,
                        allowEscapeKey: false,
                        ...
                        console.log(result);
                        if (result.isConfirmed) {
                            window.location.href="/admin/monthly-box";
                        }
                    });
                }
            },
